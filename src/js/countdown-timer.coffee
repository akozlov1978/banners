class CountdownTimer
    ...
    constructor: (_selector, _title, _finalDate, _options)->
        @options = _options ||
            days:    'дд'
            hours:   'чч'
            minutes: 'мм'
    ...
    update: ()->
        ...
        removeClass = ($items, className) -> $items.forEach ($item) -> $item.classList.remove className
        addClass = ($items, className) -> $items.forEach ($item) -> $item.classList.add className
        ...
        for name, title of @options
            ...
            $parts[0].children[0].dataset.next = value[0]
            $parts[0].children[1].dataset.next = value[0]
            $parts[1].children[0].dataset.next = value[1]
            $parts[1].children[1].dataset.next = value[1]
